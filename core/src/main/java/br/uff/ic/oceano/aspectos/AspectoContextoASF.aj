/*
import br.uff.ic.oceano.asf.anotacoes.ContextoASF;
...
import br.uff.ic.oceano.core.service.impl.TransformacaoService;
...
public aspect AspectoContextoASF{
    
    
    pointcut alteraContexto(Agente agentTransient) :
        execution (@ContextoASF * *..*.*(Agente)) && args(agentTransient);
    
    Object around(Agente agentTransient) throws RuntimeException : 
        alteraContexto(agentTransient) {
throw new RuntimeException("Este metodo deve ser refatorado");

        Object retorno = proceed(agentTransient);
        ...
        if(metodo.getAnnotation(ContextoASF.class).registra()){
            ...
            List<Transformacao> transformacoes = transformacaoService.getTransformacoesPopulares(agentTransient.getTipo(), agentTransient.getQuantidadeTransformacoes());
